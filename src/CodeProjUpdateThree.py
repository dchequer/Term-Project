...
combostyle = ttk.Style(); combostyle.theme_create('combostyle', parent='alt', settings = {'TCombobox':{'configure':{'fieldbackground': 'black', 'foreground': 'white'}}}); combostyle.theme_use('combostyle')
...
def getCSVFile():
    ...
    getCSVHeaders(df)
    placeButtons(69)
    tb = Button(root, text = "Change to " + timeName + " time", command = changeToLocalOrUTCTime, bg="#000000", fg="#00FFFF")
    tb.grid(column=0, row=(6+lengthOfHeaders))
    tb.bind("<Enter>", func=lambda e: tb.config(bg="#FFFFFF", fg="#000000", cursor='crosshair'))
    tb.bind("<Leave>", func=lambda e: tb.config(bg="#000000", fg="#FFFFFF", cursor='arrow'))
    buttonsAndChecksList.append(tb)
    gg = Button(root, text = "Get Graphs & Data", command=lambda: getGraphs(df), bg="#000000", fg="#00FFFF")
    gg.grid(column=1, row=(6+lengthOfHeaders))
    gg.bind("<Enter>", func=lambda e: gg.config(bg="#FFFFFF", fg="#000000", cursor='crosshair'))
    gg.bind("<Leave>", func=lambda e: gg.config(bg="#000000", fg="#FFFFFF", cursor='arrow'))
...
def getCSVHeaders(df: pd.DataFrame):
    ...
    c = 0
    for h in df.head(0):
        ch = Checkbutton(root, text=h, variable=valueOfCheckList[c], onvalue=1, offvalue=0, bg='#000000', fg='#FFFFFF', selectcolor='#000000', width=20, anchor="w")
    ...
    for ss in valueOfCheckList:
        if ss.get() == 1:
            if rowCount >= 0:
                ...
                Plot.tick_params(axis='x', which='major', labelsize=10, colors='#FFFFFF')
                Plot.tick_params(axis='y', colors='#FFFFFF')
                Plot.title.set_color('#FFFFFF')
        ...
        c += 1
    if flag == 1:
        s = Scale(root, from_=0, to=timeCount, length=700, variable=scaleThree, background='#FFFFFF', highlightthickness=0, 
              highlightbackground='#FFFFFF', highlightcolor='#FFFFFF', foreground='#FFFFFF', fg='#FFFFFF', bg='#FFFFFF', activebackground='#FFFFFF', 
              bd=0, showvalue=False, command=changeInScale)
        s.place(x=1310, y=0)
        placeButtons(75)
...
def getStats(df: pd.DataFrame):
    ...
    for s in completeStatsList:
        Label(rightFrame, text=headerList[c] + ':\t\t\t' + s, bg="#000000", fg="#00FFFF", width=130, anchor="w").grid(column=2, row=((rowCount*6)+1+c))
        c += 1
    rightFrame.configure(bg="#000000")
...
def getTimeBox(df: pd.DataFrame):
    ...
    if fileCounter == 1:
        Scale(rightFrame, from_=0, to=timeCount, length=400, orient=HORIZONTAL, variable=scaleOne, background='#FFFFFF', highlightthickness=0, 
              highlightbackground='#FFFFFF', highlightcolor='#FFFFFF', foreground='#FFFFFF', fg='#FFFFFF', bg='#FFFFFF', activebackground='#FFFFFF', 
              bd=0, showvalue=False, command=changeInScale).place(x=120, y=180)
        Scale(rightFrame, from_=0, to=timeCount, length=400, orient=HORIZONTAL, variable=scaleTwo, background='#FFFFFF', highlightthickness=0, 
              highlightbackground='#FFFFFF', highlightcolor='#FFFFFF', foreground='#FFFFFF', fg='#FFFFFF', bg='#FFFFFF', activebackground='#FFFFFF', 
              bd=0, showvalue=False, command=changeInScale).place(x=530, y=180)
        if countOnce == 0:
        ...
        if flag == 0:
            ...
            placeButtons(62)
        if timeCounter == 0:
            Label(rightFrame, text=datetime.utcfromtimestamp((df['Unix Timestamp (UTC)'].values[scaleOne.get()])/1000), bg="#000000", fg="#00FFFF").place(x=15, y=155)
            Label(rightFrame, text=datetime.utcfromtimestamp((df['Unix Timestamp (UTC)'].values[(scaleTwo.get()-1)])/1000), bg="#000000", fg="#00FFFF").place(x=885, y=155)
        else:
            Label(rightFrame, text=datetime.utcfromtimestamp(((df['Unix Timestamp (UTC)'].values[scaleOne.get()]+(df['Timezone (minutes)'].values[(scaleTwo.get()-1)]*60000)))/1000), bg="#000000", fg="#00FFFF").place(x=15, y=155)
            Label(rightFrame, text=datetime.utcfromtimestamp(((df['Unix Timestamp (UTC)'].values[(scaleTwo.get()-1)]+(df['Timezone (minutes)'].values[(scaleTwo.get()-1)]*60000)))/1000), bg="#000000", fg="#00FFFF").place(x=885, y=155)
        distCount = 1
        if timeCount > 730:
            distCount = 2
        lengthOfLabelLeft = 140+(scaleOne.get()/distCount)
        lengthOfLabelRight = 870-((timeCount-scaleTwo.get())/distCount)
        Label(rightFrame, text='', height=10, width=1, background='#FFFFFF').place(x=lengthOfLabelLeft, y=20)
        Label(rightFrame, text='', height=10, width=1, background='#FFFFFF').place(x=lengthOfLabelRight, y=20)
        lengthOfDottedLines = lengthOfLabelRight-lengthOfLabelLeft+10
        C1 = Canvas(rightFrame, width=lengthOfDottedLines, height=8, background='#000000')
        C1.create_line(10,5,lengthOfDottedLines,5, dash=(5,1), fill='#FFFFFF')
        C1.place(x=lengthOfLabelLeft, y=20)
        C2 = Canvas(rightFrame, width=lengthOfDottedLines, height=8, background='#000000')
        C2.create_line(10,5,lengthOfDottedLines,5, dash=(5,1), fill='#FFFFFF')
...
titleLabel = Label(root, text="CSE 350 Data Visualization Project", bg="#000000", fg="#00FFFF").grid(column=0, row=0, columnspan=2)
filebutton = Button(root, text = "Upload CSV File", command = getCSVFile, bg="#000000", fg="#00FFFF")
filebutton.grid(column=0, row=1, columnspan=2)
filebutton.bind("<Enter>", func=lambda e: filebutton.config(bg="#FFFFFF", fg="#000000", cursor='crosshair'))
filebutton.bind("<Leave>", func=lambda e: filebutton.config(bg="#000000", fg="#FFFFFF", cursor='arrow'))
dateLabel1 = Label(root, text="Select Start Date: ", bg="#000000", fg="#00FFFF").grid(column=0, row=3)
dateLabel2 = Label(root, text="Select End Date: ", bg="#000000", fg="#00FFFF").grid(column=1, row=3)
userEntryStartTime = Entry(root, textvariable=userEnterStartDate, bg="#000000", fg="#00FFFF").grid(column=0, row=4, ipady=15)
userEntryEndTime = Entry(root, textvariable=userEnterEndDate, bg="#000000", fg="#00FFFF").grid(column=1, row=4, ipady=15)
headerLabel = Label(root, text="Select Headers to Graph", bg="#000000", fg="#00FFFF").grid(column=0, row=5)
pickGraphForHeaderLabel = Label(root, text="Select Graph Format", bg="#000000", fg="#00FFFF").grid(column=1, row=5)
rightFrame = Frame(root)
